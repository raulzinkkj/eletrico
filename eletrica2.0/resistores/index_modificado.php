...
<body>
    ...
    <script>
        ...
        function renderBlocks() {
            ...
            blocks.forEach((block, bIdx) => {
                ...
                <div class="input-group input-group-sm mb-1">
                    ...
                    <input type="number" class="form-control" value="${v}" step="any" oninput="updateResistor(${block.id}, ${rIdx}, this.value)">
                    <span class="input-group-text">Ω</span>
                    <button class="btn btn-outline-danger btn-sm" onclick="removeResistorFromBlock(${block.id}, ${rIdx})"><i class="bi bi-trash"></i></button>
                </div>
                `).join('');
        ...
        function desenharCircuito(V, Req, I) {
            ...
            ctx.beginPath();
            ctx.lineWidth = 4;
            ctx.moveTo(batX - 15, batY - 15);
            ctx.lineTo(batX + 15, batY - 15); // Positivo
            ctx.stroke();
            ctx.beginPath();
            ctx.lineWidth = 2;
            ctx.moveTo(batX - 8, batY + 15);
            ctx.lineTo(batX + 8, batY + 15); // Negativo
            ctx.stroke();
            ...
            blocks.forEach((block, idx) => {
                if (block.tipo === 'serie') {
                    ...
                    ctx.beginPath();
                    ctx.moveTo(curX, startY);
                    ctx.lineTo(curX, pStartY);
                    ctx.lineTo(curX, pStartY + totalH);
                    ctx.stroke();
                    ...
                    block.valores.forEach((v, i) => {
                        ...
                        ctx.beginPath();
                        ctx.moveTo(curX, py);
                        ctx.lineTo(curX + 10, py);
                        ctx.stroke();
                        desenharResistor(ctx, curX + 10, py, 40, v, false);
                        ctx.beginPath();
                        ctx.moveTo(curX + 50, py);
                        ctx.lineTo(curX + 60, py);
                        ctx.stroke();
                    });

                    ctx.beginPath();
                    ctx.moveTo(curX + 60, pStartY);
                    ctx.lineTo(curX + 60, pStartY + totalH);
                    ctx.stroke();
                    ctx.beginPath();
                    ctx.moveTo(curX + 60, startY);
                    ctx.lineTo(curX + 75, startY);
                    ctx.stroke();
                    curX += 75;
                }
            });
